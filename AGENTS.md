...
| `npm run test:e2e` (from `manga-studio/`) | Playwright browser smoke tests (`tests/e2e/`) — starts its own dev server on port 3100; not part of the standard four checks below (slower, needs a real browser), but run it too when touching top-level dialogs/navigation/anything a unit test can't see render |
