...
Background removal is a third independent provider kind with its own encrypted HttpOnly cookie. AI Settings offers a remove.bg quick preset, a **ComfyUI (local)** preset, and a declarative Custom API mapping (URL or base64 reference, response URL/base64, sync or polling). The user supplies only that provider's endpoint/key in the UI. `APP_ENCRYPTION_KEY` remains a one-time operator infrastructure secret used to encrypt all user BYOK cookies; it is never a user provider key. Optional `BACKGROUND_REMOVAL_*` environment variables are deployment-wide fallbacks only.

**ComfyUI as a background-removal fallback** (`createComfyUiBackgroundRemovalProvider`, `ai/providers/comfyui.ts`) — a third, independent capability from that same file's image-generation adapter; a creator can point Image Generation at one ComfyUI instance and this fallback at a completely different one (or the same one, on a different endpoint path — irrelevant to ComfyUI, which routes purely by node graph, not URL path). Reuses the file's existing SSRF-guarded `uploadImage`/`submitPrompt`/`pollHistory`/`fetchImageBytes` helpers wholesale — no second outbound-fetch path. Its own minimal `ComfyUiBackgroundRemovalConfig` (`baseUrl`/`apiKey`/`name` only) rather than the image adapter's `ComfyUiConfig`, which requires a checkpoint `model` this capability has no use for — the Model field already stays hidden for the whole `kind: "background"` card, and so does the (irrelevant here) "Advanced — ComfyUI settings" section (steps/CFG/LoRA/ControlNet/IPAdapter), gated on `kind !== "background"` alongside the existing `providerType === "comfyui"` check.

`buildBackgroundRemovalWorkflow` builds an independent graph (node ids `60`-`63`, reusing `SaveImage` id `"9"` so the shared `pollHistory` helper — which reads `outputs["9"]` — needs no change): `LoadImage` → **`InspyrenetRembg`** (from the `ComfyUI-Inspyrenet-Rembg` custom node pack — NOT part of a vanilla ComfyUI install; class name/inputs/outputs verified directly against `Inspyrenet_Rembg.py`, not guessed) → **`JoinImageWithAlpha`** (a core ComfyUI node merging the rembg node's `IMAGE`+`MASK` outputs into one real-alpha `IMAGE`) → `SaveImage`. The `JoinImageWithAlpha` step specifically was not verified against a live instance (none was available this session) — a wrong node/field name surfaces as ComfyUI's own `/prompt` rejection, never silent wrong output, the same safety net used throughout this file for every other unverified node shape. The result still passes through the shared `validatedProviderResult` (`assets/providers/validateResult.ts`) — the same alpha-contract check every other background-removal provider's output gets, so a malformed cutout is rejected the same way regardless of which provider produced it.
